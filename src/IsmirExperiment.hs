...
reportSLFP :: (_) => FilePath -> SLFP a b -> IO ()
reportSLFP resultDir slfp = do
...
runExperiment readCorpus getName getTree inputFile outputDir = do
    ...
    reportSLFP outputDir $ mkCorpusSLFP getName getTree ps
...
ismirExperiment = do
    ...
    runExperiment
        ...
        title
        tRule
        (p <> "/experiment/data/treebank.json")
        (p <> "/experiment/result/")
