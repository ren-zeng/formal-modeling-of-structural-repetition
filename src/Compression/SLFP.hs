...
import Compression.Meta (Meta, RepSymbol (..), useMeta, inferMeta)
...
rRewriteTop :: (a, Meta) -> Tree (Abstraction a) -> Tree (Abstraction a)
rRewriteTop (x, m) (Node r ts) = Node (r `With` (x, catMaybes $ zipWith pickByMeta m (rootLabel <$> ts))) (foldMap subForest ts) where
...
data SLTP a = SLTP
  { compressedTree :: Tree (Abstraction a),
    patterns :: Map.Map (Abstraction a) (Pattern (Abstraction a)),
    metas :: Map.Map a Meta
  }
  deriving (Show, Eq)
...
compressL :: (PrintfType a, Ord a, a~String) => Int -> p -> Map.Map a Meta -> SLTP a -> SLTP a
...
performPRewrite :: (a~String, Ord a) => Int -> Pattern (Abstraction a) -> [Location] -> SLTP a -> SLTP a
performPRewrite n p locs (SLTP t dP dR) = SLTP t' dP' dR'
  where
    name = Abstraction $ printf "P%d_%d" n (Map.size dP + 1)
    t' = pRewriteAll (name, p) t locs
    (dP', dR') = (Map.insert name p dP, dR)

performRRewrite :: (a~String, Ord a) => Int -> Map.Map a Meta -> Meta -> [Location] -> SLTP a -> SLTP a
...
data SLFP a b = SLFP
  { sltps :: Map.Map b (SLTP a),
    globalPatterns :: Map.Map (Abstraction a) (Pattern (Abstraction a)),
    globalMetas :: Map.Map a Meta,
...
localMetas :: (_) => SLFP a b -> Map.Map a Meta
...
compressG :: (Ord b, PrintfType a, Ord a, a ~ String) => SLFP a b -> SLFP a b
compressG g@(SLFP dE dpG drG dA) = case (mp, mr) of
  ...
  where
    ...
    compressByP p plocs =
      SLFP
        (uncurry (flip $ pRewriteTreeOnly (nameP p, p)) <$> zipDict dE plocs)
    ...
    nameP p = if p `elem` dpG then lookupByValue p dpG else Abstraction (freshSymbol "PG" dE dpG drG)
    nameR r = if r `elem` drG then lookupByValue r drG else freshSymbol "RG" dE dpG drG

    
compressGSteps :: Ord b => SLFP String b -> [SLFP String b]
...
deCompressL :: (Ord a) => Map.Map (Abstraction a) (Pattern (Abstraction a)) -> Map.Map a Meta -> Map.Map (Abstraction a) Int -> SLTP a -> SLTP a
...
deCompressTree ::
  (Ord k) =>
  (Abstraction k -> Pattern (Abstraction k)) ->
  (k -> Meta) ->
  (Abstraction k -> Int) ->
  Tree (Abstraction k) ->
  Tree (Abstraction k)
...
inferArity d dMeta dA a = case a of
  ...
  Abstraction x -> inferArity d dMeta dA l + inferArity d dMeta dA r - 1
    where
      (Comp _ l r) = d (Abstraction x)
